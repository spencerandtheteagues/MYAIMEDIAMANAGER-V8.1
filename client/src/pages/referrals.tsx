...
export default function Referrals() {
  ...
  // Simulated referral data - would come from API
  const referralData: ReferralData = {
    referralCode: user?.id ? `${user.fullName?.split(' ')[0]?.toUpperCase()}${user.id.slice(-4).toUpperCase()}` : "REFER123",
    totalReferrals: 8,
    freeTrialReferrals: 5,
    paidReferrals: 3,
    totalCreditsEarned: 265,
    currentTierProgress: 3,
    nextMilestone: 5,
    referralHistory: [
      { id: "1", date: "2024-01-15", type: "free", credits: 25, status: "completed" },
      { id: "2", date: "2024-01-18", type: "free", credits: 30, status: "completed" },
      { id: "3", date: "2024-01-20", type: "paid", credits: 100, status: "completed" },
      { id: "4", date: "2024-01-22", type: "free", credits: 35, status: "completed" },
      { id: "5", date: "2024-01-25", type: "free", credits: 40, status: "completed" },
      { id: "6", date: "2024-01-28", type: "paid", credits: 100, status: "pending" },
      { id: "7", date: "2024-01-30", type: "free", credits: 45, status: "completed" },
      { id: "8", date: "2024-02-01", type: "paid", credits: 100, status: "pending" },
    ]
  };
  ...
  const handleCopyLink = () => {
    const referralLink = `https://myaimediamgr.com/signup?ref=${referralData.referralCode}`;
  ...
  const progressToFreeSubscription = Math.min((referralData.paidReferrals / 25) * 100, 100);
        ...
        <Card className="bg-gradient-to-br from-primary/10 to-accent/10">
          ...
          <CardContent className="space-y-3">
            ...
            <div className="flex justify-between">
              ...
              <span className="font-bold">{referralData.freeTrialReferrals}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-sm text-muted-foreground">Paid Subscriptions</span>
              <span className="font-bold">{referralData.paidReferrals}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-sm text-muted-foreground">Credits Earned</span>
              <span className="font-bold text-primary">{referralData.totalCreditsEarned}</span>
            </div>
          </CardContent>
        </Card>
      </div>
            ...
            <CardContent>
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
                {freeTrialRewards.map((credits, index) => {
                  const isCompleted = index < referralData.freeTrialReferrals;
                  return (
                    <div 
                      key={index}
            ...
            <CardContent className="space-y-4">
              ...
              <div className="p-4 bg-gradient-to-r from-purple-50 to-pink-50 dark:from-purple-950 dark:to-pink-950 rounded-lg">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-sm font-bold">{referralData.paidReferrals}/25</span>
                </div>
                <Progress value={progressToFreeSubscription} className="h-3" />
                <p className="text-xs text-muted-foreground mt-2">
                  {25 - referralData.paidReferrals} more paid referrals needed for a free month!
                </p>
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className={`p-4 border rounded-lg ${referralData.totalReferrals >= 1 ? 'bg-bronze-50 border-bronze-200' : ''}`}>
                  ...
                  {referralData.totalReferrals >= 1 && <Badge className="mt-2">Achieved</Badge>}
                </div>
                
                <div className={`p-4 border rounded-lg ${referralData.totalReferrals >= 5 ? 'bg-silver-50 border-silver-200' : ''}`}>
                  <div className="flex items-center gap-2 mb-2">
                  ...
                  {referralData.totalReferrals >= 5 && <Badge className="mt-2">Achieved</Badge>}
                </div>
                
                <div className={`p-4 border rounded-lg ${referralData.totalReferrals >= 10 ? 'bg-gold-50 border-gold-200' : ''}`}>
                  <div className="flex items-center gap-2 mb-2">
                    <Crown className="w-5 h-5 text-gold-500" />
                    <span className="font-medium">Gold Referrer</span>
                  </div>
                  <p className="text-sm text-muted-foreground">10+ referrals</p>
                  {referralData.totalReferrals >= 10 && <Badge className="mt-2">Achieved</Badge>}
                </div>
              </div>
            </CardContent>
          </Card>
                ...
                {referralData.referralHistory.map((referral) => (
                  <div key={referral.id} className="flex items-center justify-between p-3 bg-muted/50 rounded-lg">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="font-medium">
                          {referral.type === 'paid' ? 'Paid Subscription' : 'Free Trial'}
                        </p>
                    ...
                    <div className="text-right">
                      <p className="font-bold text-primary">+{referral.credits} credits</p>
