...
// Fix enterprise tier constraint and upgrade admin users
router.post("/fix-enterprise-tier", async (req, res) => {
  try {
    const adminUser = (req as any).adminUser;
    const adminId = (req as any).adminId;

    console.log('Enterprise tier fix requested by:', adminUser?.email);
    ...
    try {
      // Step 1: Update the constraint to include enterprise
      console.log('Updating tier constraint to include enterprise...');
      ...
      res.json({
        success: true,
        message: `Fixed enterprise tier constraint and updated ${updatedCount} admin accounts`,
        totalAdmins: adminUsersResult.rows.length,
        updatedCount,
        enterpriseAdmins: parseInt(verifyResult.rows[0].enterprise_admins),
        updates
      });
    ...
    res.status(500).json({
      success: false,
      message: "Error fixing enterprise tier: " + error.message
    });
  }
});
